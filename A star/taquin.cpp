...
booleen testBut(char** config, char(*but)[3]){
    for(int i=0; i<3; i++){
        for(int j=0; j<3; j++){
            if(config[i][j] != but[i][j])
                return false;
...
void permuteridem(char** config, int x1, int y1, int x2, int y2) {
    // will mutate config!
    if(x2 >= 0 && x2 < 3 && y2 >= 0 && y2 < 3){
        char temp = config[x2][y2];
        config[x2][y2] = config[x1][y1];
...
void copier(char (*src)[3], char** dest){
    for(int i=0; i<3; i++){
        for(int j=0; j<3; j++){
            dest[i][j] = src[i][j];
        }
...
void copieridem(char** src, char** dest){
    for(int i=0; i<3; i++){
        for(int j=0; j<3; j++){
            dest[i][j] = src[i][j];
        }
